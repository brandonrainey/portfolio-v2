        ...
        <div className="flex items-center">
          {project.skills.map((skill: string, index) =>
            skills.map(
              (s) =>
                s.name === skill && (
                  <div className={`sm:h-14 w-10 sm:w-14 h-10 p-1 flex items-center justify-center rounded-full border-2 border-white/50 bg-[#444059] -ml-2 shadow shadow-slate-700`} key={index}>
                    <img
                      src={s.icon}
